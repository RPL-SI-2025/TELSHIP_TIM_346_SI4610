...
@section('main')
    ...
    <div class="d-flex" style="min-height: 100vh">
        ...
        <div class="flex-grow-1">
            ...
            <div class="container py-4">
                    ...
                    <div class="text-end">
                        @if (!empty($lowongans->userMentor->mitra->logo_perusahaan))
                            <img src="{{ asset('storage/' . $lowongans->userMentor->mitra->logo_perusahaan) }}"
                                ...
                                onerror="this.onerror=null; this.src='{{ asset('app-assets/img/ICON.svg') }}';">
                        @else
                ...
                <div class="info-section">
                    <h4>Mahasiswa yang Lolos</h4>
                    <div class="info-detail">
                        <table id="mahasiswaLolos" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <th>No</th>
                            ...
                            <tbody>
                                <!-- Data Mahasiswa Lolos akan dimuat di sini -->
                            </tbody>
                        </table>
                    </div>
                </div>

                @foreach ($lamaranLolos as $lamaran)
                    @php
    ...
    <script>
        $(document).ready(function() {
            $('#mahasiswaLolos').DataTable({
                processing: true,
                serverSide: false,
                ajax: {
                    url: "{{ route('lamaran.getLolos', ['lowonganId' => $lowongans->id_lowongan]) }}",
                    type: 'GET',
                    dataSrc: 'data'
                },
                columns: [{
